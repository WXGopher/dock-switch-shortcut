...
**中文：** 首次启动默认暂停。用户点击菜单中的 **Enable Dock Shortcuts** 后，在 **系统设置 → 隐私与安全性 → 辅助功能** 中允许 **Dock Switcher**；列表中没有时用 **+** 添加已安装的 App。授权后重新打开 App 菜单，快捷键会恢复；不要为了恢复而再次切换已勾选的启用开关。

**English:** First launch starts paused. The user chooses **Enable Dock Shortcuts**, then allows **Dock Switcher** under **System Settings → Privacy & Security → Accessibility**. If absent, use **+** to add the installed app. Reopen the app menu after granting access to resume shortcuts; do not toggle an already checked enable setting merely to resume.

**中文：** 更新或重新构建后，如果系统设置中的开关已打开，App 却仍提示未授权，旧授权可能未匹配新版。ad-hoc 签名的身份与具体构建绑定，见 [Apple 签名说明](https://developer.apple.com/documentation/technotes/tn3127-inside-code-signing-requirements)。先退出 Dock Switcher，再由用户在辅助功能列表中选中旧条目并点 **−**，随后点 **+** 添加 **/Applications/Dock Switcher.app**，打开开关，重新启动 App 并点击 **Remap Dock Apps**。不要误选源码构建目录或备份中的副本；仅看到开关已打开不代表运行中的新版已获得授权。

**English:** After an update or rebuild, an enabled switch in System Settings can coexist with an app reporting no permission: the old grant may not match the new build. An ad-hoc signature's identity is tied to a specific build; see [Apple's signing explanation](https://developer.apple.com/documentation/technotes/tn3127-inside-code-signing-requirements). Quit Dock Switcher, then have the user select its old Accessibility entry and click **−**. Click **+** to add **/Applications/Dock Switcher.app**, enable its switch, relaunch the app, and choose **Remap Dock Apps**. Select the installed app rather than a build-directory or backup copy. The switch alone does not prove that the running build is authorized.
